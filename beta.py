...
class charRNN:
    ...
    def __init__(self,
        ...
        cell_fw, self.initial_state_fw = lstm_layer(lstm_size,layer_num,batch_size,self.keep_prob)
        cell_bw, self.initial_state_bw = lstm_layer(lstm_size,layer_num,batch_size,self.keep_prob)
        ...
        outputs, state = tf.nn.bidirectional_dynamic_rnn(cell, in_one_hot, initial_state = self.initial_state)
...
print ('Number of characters in document {0}'.format(len(text)))  
vocab = set(text)
